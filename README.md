...
## Запуск проекта
### Для соискателя: 
Необходимо загрузить резюме в формате .docx в папку rezyume, предварительно удалив из нее ненужные файлы (сейчас там находится пример резюме), запустить алгоритм через файл main.py, выбрать роль Соискатель:
```
python main.py
```
### Пример работы
...
main.py|Основной скрипт, в котором запускаются main_hr.py и main_candidate.py, в зависимости от выбора пользователя
